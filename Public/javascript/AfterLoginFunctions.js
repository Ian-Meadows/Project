...
function buildHtmlTable(selector) {
      ...
      $.ajax('/GetGames/Get', function(data) {
        ...
        for (var i = 0; i < games.length; i++) {
          ...
          cellValue = '<form action=\"MakeBetPage\" method=\"get\"><input type="hidden" name=\'id\' value = '+games[i].id+'><input type="hidden" name=\"home\" value = '+games[i].home+'><input type="hidden" name=\"visitor\" value = '+games[i].visitor+'><input type="submit" name=\"bet\" value=\"Bet\" /></form>';
          row$.append($('<td/>').html(cellValue));
